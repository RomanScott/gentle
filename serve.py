...
class TranscriptionsController(Resource):
    ...
    def render_POST(self, req):
        def quitGentle(restart):
            ...
            if restart is True:
                if os.name == "nt":
                    subprocess.Popen(RESTART_CMD_WIN, shell=True, stdin=None, stdout=None, stderr=None, close_fds=True, cwd=os.path.dirname(sys.executable))
                else:
                    subprocess.Popen(RESTART_CMD_MAC, shell=True, cwd=os.path.dirname(sys.executable))
...
if __name__=='__main__':
    ...
    parser.add_argument('--nthreads', default=1 if multiprocessing.cpu_count() is 1 else int(multiprocessing.cpu_count() * 0.75), type=int,
